...
class DataOrchestrator:
    def __init__(self):
        ...
        self.stats: Dict[str, Dict] = {}  # For storing insertion statistics
    ...
    async def process_day(self, instrument: str, day: date) -> int:
        """Process a full day of data"""
        tasks = []
        for hour in range(24):
            if self.should_download_hour(instrument, day, hour):
                tasks.append(self.process_hour(instrument, day, hour))
                
        if tasks:
            # Process hours concurrently but limited
            results = await asyncio.gather(*tasks, return_exceptions=True)
            
            # Count successful ticks
            total_ticks = 0
            for result in results:
                if isinstance(result, int):
                    total_ticks += result
                    
            logging.info(f"Processed {total_ticks} ticks for {instrument} on {day}")
            return total_ticks
    ...
    async def run(self):
        ...
        for instrument in self.config['INSTRUMENTS']:
            ...
            async def process_with_limit(day):
                async with semaphore:
                    ticks = await self.process_day(instrument, day)
                    self.stats[instrument]['total_ticks'] += ticks if ticks else 0
                    self.stats[instrument]['days_processed'] += 1
                    pbar.update(1)
                    pbar.set_description(f"Processing {instrument} - Day {self.stats[instrument]['days_processed']}/{len(dates)}")
            
            await asyncio.gather(*[process_with_limit(day) for day in dates])
            pbar.close()
            
            # Print statistics for this instrument
